...
class GA(object):
    ...
    def genetic_algorithm(self):
        ...
        # PolyListProcessor.show_poly_list(self.width,self.global_best_sequence)
        # self.plotRecord()
        ...
        if self.ga_multi == True:
            ...
            for i in range(0, len(self.pop)):
                ...
                length_results.append(
                        [i, results[i], 1000 / results[i], PolyListProcessor.getPolyListIndex(self.pop[i])])
        else:
            for i in range(0, len(self.pop)):
    ...
    def getLength(self, poly_list):
        length = PolyListProcessor.packingLength(poly_list, self.history_index_list, self.history_length_list,
                                                 self.width, NFPAssistant=self.NFPAssistant)
        return length
    ...
    def getNextGeneration(self):
        ...
        for i in range(0, self.pop_size - self.elite_size):
            children.append(self.breed(children[random.randint(0, self.elite_size - 1)],
                                       children[random.randint(0, self.elite_size - 1)]))
    ...
    def breed(self, parent1, parent2):
        ...
        return PolyListProcessor.getPolysByIndex(child1_index, self.poly_list) + PolyListProcessor.getPolysByIndex(
                child2_index, self.poly_list)
        ...
        for swapped in range(len(individual)):
            if (random.random() < self.mutate_rate):
                ...
                if random.random() <= 0.5:
                    ...
                    individual = PolyListProcessor.random_rotate(individual, self.minimal_rotation, swapped)
        return individual
...
class SA(object):
    ...
    def new_poly_list(self):
        ...
        if random.random() <= 1:
            ...
            self.new_poly_list = PolyListProcessor.random_rotate(self.cur_poly_list, self.min_angle, choose_id)

    def run(self):
        initial_length = PolyListProcessor.packingLength(self.cur_poly_list, self.history_index_list,
        ...
        while self.temp_now > self.temp_end:
            ...
            cur_length = PolyListProcessor.packingLength(self.cur_poly_list, self.history_index_list,
                                                         self.history_length_list, self.width,
            ...
            for i in range(self.loop_times):
                ...
                new_length = PolyListProcessor.packingLength(self.new_poly_list, self.history_index_list,
                                                             self.history_length_list, self.width,
                                                             NFPAssistant=self.NFPAssistant)
        ...
        PolyListProcessor.show_poly_list(self.width, global_best_list)

        self.show_best_result(temp_lowest_length_list, global_lowest_length_list)

    def show_best_result(self, list1, list2):
        plt.figure(1)
        plt.subplot(311)
...
def main():
    ...
    # SA(poly_list)
    # GetBestSeq(1000,get_convex(num=5),"decrease")
    end_time = datetime.datetime.now()
    print("算法消耗时间：", end_time - start_time)
    blf = BottomLeftFill(1500, polys, vertical=False, NFPAssistant=nfp_assistant)
    blf.show_all()
